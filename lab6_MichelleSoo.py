...
def reverse_str(s):
    ...
    for i in range(len(s)):
        # newString.append(s[reverseIndex])
        i = i - 1
        newString = newString + s[(i - 1)] 
    return newString

# Your unit tests
assert(reverse_str("Abd"))
assert(reverse_str("COMP115"))
assert(reverse_str("hellothere"))
...
def count_vowels(s):
    ...
    for i in range(len(lowerCase)):
        if lowerCase[i] in ("aeiouAEIOU"):
            sum += 1
    return sum

# # Your unit tests
assert(count_vowels("Apple"))
assert(count_vowels("pEar"))
...
assert(remove_duplicates("Apple"))
assert(remove_duplicates("Popsipple"))
assert(remove_duplicates("pear"))
...
# Your unit tests
assert(find_index("Abdcccklsdmflsf", 'm'))
assert(find_index("Abdccc", 'c'))
...
def project_completion_day(day, days_to_completion):
    ...
    for i in range(len(days_week)):
        if days_week[i] == day:
            start = i
    finish = start + days_to_completion
    return days_week[finish % 7]

# Your unit tests
assert(project_completion_day('Tuesday', 8))
assert(project_completion_day('Monday', 4))
assert(project_completion_day('Saturday', 2))
