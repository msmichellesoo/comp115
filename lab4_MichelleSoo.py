...
def convert_american_dollars(american_dollars):
    canadian_dollars = round((float(american_dollars) * 1.34), 2)
...
def evens(nums):
    ...
    return empty_list 
# result = evens([1, 2, 3, 4])
# print(result)
evens

# Write and pass your units tests (5 marks)
assert evens([1, 2, 3, 4])
assert evens([2, 4, 6, 7, 8])
...
assert sum_of_squares([1, 2, 3, 4])
assert sum_of_squares([2, 6, 7, 3, 9])
...
assert squares([1, 2, 3, 4])
assert squares([2, 3, 4, 5])
assert squares([8, 2, 8, 4, 9])
"""
